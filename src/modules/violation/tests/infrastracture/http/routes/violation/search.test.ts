...
describe("GET /api/v1/violation/search", () => {
  ...
  it("should return violation with proper pagination", async () => {
    ...
    expect(result2.violation.length).toBe(3);
    expect(result2.hasNextPage).toBe(false);
    expect(result2.hasPreviousPage).toBe(true);
    expect(result2.totalPages).toBe(2);
    expect((result2.violation as IViolationDTO[]).map((violation) => violation.id)).not.toContain(
      seededViolation1.id
    );
  });
});
