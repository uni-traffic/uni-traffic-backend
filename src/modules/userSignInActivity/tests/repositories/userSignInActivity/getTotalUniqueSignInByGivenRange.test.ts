...
} from "../../../src/repositories/userSignInActivityRepository";
import { seedUserSignInActivity } from "../../utils/seedUserSignInActivity";

describe("UserSignInActivityRepository.getTotalUniqueSignInByGivenRange", () => {
  let userSignInActivityRepository: IUserSignInActivityRepository;

  beforeAll(() => {
    userSignInActivityRepository = new UserSignInActivityRepository();
  });
  ...
  it("should return the correct count of user sign in activity by DAY on given range", async () => {
    ...
    const result = await userSignInActivityRepository.getTotalUniqueSignInByGivenRange({
      startDate: new Date("2024-05-01T00:00:00.000Z"),
      endDate: new Date("2024-05-05T23:59:59.999Z"),
      type: "DAY"
    });
