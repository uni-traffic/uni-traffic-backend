...
export const createVehicleApplicationPersistenceData = ({
  id = uuid(),
  schoolId = uuid(),
  ...
  applicantId,
  applicant
}: Partial<IVehicleApplicationRawObject>): IVehicleApplicationRawObject => {
  const applicantUser = applicant ? applicant : createUserPersistenceData({});
  const applicantUserId = applicantId ? applicantId : applicantUser.id;
